# A second hand-built combined taxonomic reference table, complementing sample_taxonomic_resources()
...
#   extremely common in real zoological nomenclature, distinct from sample_taxonomic_resources()'s
#   Boronia (Valvatae) case where the subgenus has its own, different name
# - *two* synonyms for the same accepted species (sample_taxonomic_resources() only ever has one per
...
# - a subspecies-rank row distinct from sample_taxonomic_resources()'s "variety" example
#
...
sample_invert_taxonomic_resources <- function() {
  tibble::tribble(
