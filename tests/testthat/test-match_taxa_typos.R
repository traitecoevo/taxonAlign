...
# sample_invert_taxon_resources() (helper-invert-typo-fixtures.R) -- complementing the more
# structural, plant-flavoured tests in test-align_taxa.R/test-match_taxa.R (which use
# sample_taxon_resources()). Every scenario here was first verified interactively against the real
...
test_that("every edit-distance type of typo still fuzzy-matches the correct species", {
  resources <- prepare_taxonomic_resources(sample_invert_taxon_resources())
...
test_that("the nominotypical subgenus bracket convention resolves to the species it wraps", {
  ...
  # distinct from sample_taxon_resources()'s Boronia (Valvatae) case, where the subgenus has a
